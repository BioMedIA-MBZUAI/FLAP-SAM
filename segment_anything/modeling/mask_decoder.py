...
class MaskDecoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        # self.output_upscaling_lora = nn.Sequential(
        #     nn.ConvTranspose2d(transformer_dim, transformer_dim // 4, kernel_size=2, stride=2),
        #     LayerNorm2d(transformer_dim // 4),
        #     activation(),
        #     nn.ConvTranspose2d(transformer_dim // 4, transformer_dim // 8, kernel_size=2, stride=2),
        ...
        # self.output_hypernetworks_mlps_lora = nn.ModuleList(
        #     [
        #         MLP(transformer_dim, transformer_dim, transformer_dim // 32, 3)
        #         for i in range(self.num_mask_tokens)
        #     ]
    ...
    def predict_masks(
        ...
        # print(src.shape)
        upscaled_embedding = self.output_upscaling(src)# + self.output_upscaling_lora(src)
        # upscaled_embedding = self.output_upscaling_lora(src)
        # print(upscaled_embedding.shape)
        hyper_in_list: List[torch.Tensor] = []
        for i in range(self.num_mask_tokens):
            hyper_in_list.append(self.output_hypernetworks_mlps[i](mask_tokens_out[:, i, :]))
            # hyper_in_list.append(self.output_hypernetworks_mlps[i](mask_tokens_out[:, i, :]) + self.output_hypernetworks_mlps_lora[i](mask_tokens_out[:, i, :]))
            # hyper_in_list.append(self.output_hypernetworks_mlps_lora[i](mask_tokens_out[:, i, :]))

...
class ImageEncoderViT(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = self.patch_embed(x)  #+ self.patch_embed_lora(x) # pre embed: [1, 3, 1024, 1024], post embed: [1, 64, 64, 768]
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        if self.enable_dora:
            self.dora_m_q = nn.Parameter(self.qkv.weight[:dim].norm(p=2, dim=0, keepdim=True))
            self.dora_m_v = nn.Parameter(self.qkv.weight[-dim:].norm(p=2, dim=0, keepdim=True))
        ...
        if num_lora > 0:
            ...
            for _ in range(num_lora):
                self.lora_w_a_q.append(nn.Linear(dim, r, bias=False))
                self.lora_w_b_q.append(nn.Linear(r, dim, bias=False))
                self.lora_w_a_v.append(nn.Linear(dim, r, bias=False))
                self.lora_w_b_v.append(nn.Linear(r, dim, bias=False))
        elif num_lora < 0:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # DoRA
        if self.enable_dora:
            numerator_q = self.qkv.weight[:C] + (self.lora_w_b_q[0].weight @ self.lora_w_a_q[0].weight) * alphas[0]
            denominator_q = numerator_q.norm(p=2, dim=0, keepdim=True)
            weight_q = self.dora_m_q * (numerator_q / denominator_q)
            qkv[:, :, :, :C] = F.linear(x, weight_q, self.qkv.bias[:C])
            numerator_v = self.qkv.weight[-C:] + (self.lora_w_b_v[0].weight @ self.lora_w_a_v[0].weight) * alphas[0]
            denominator_v = numerator_v.norm(p=2, dim=0, keepdim=True)
            weight_v = self.dora_m_v * (numerator_v / denominator_v)
            qkv[:, :, :, -C:] = F.linear(x, weight_v, self.qkv.bias[-C:])
        else:
            # LoRA
            if self.num_lora >= 1:
                for i in range(self.num_lora):
                    qkv[:, :, :, :C] += self.lora_w_b_q[i](self.lora_w_a_q[i](x)) * alphas[i]
                    qkv[:, :, :, -C:] += self.lora_w_b_v[i](self.lora_w_a_v[i](x)) * alphas[i]

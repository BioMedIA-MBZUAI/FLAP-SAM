...
class RandomGenerator(object):
    ...
    def __call__(self, sample):
        ...
        if x != self.output_size[0] or y != self.output_size[1]:
            image = ndimage.zoom(image, (self.output_size[0] / x, self.output_size[1] / y, 1.0), order=3)
            label = ndimage.zoom(label, (self.output_size[0] / x, self.output_size[1] / y, 1.0), order=0)
        label_h, label_w, label_d = label.shape
        low_res_label = ndimage.zoom(label, (self.low_res[0] / label_h, self.low_res[1] / label_w, 1.0), order=0)
...
class dataset_reader(Dataset):
    def __init__(self, base_dir, split, num_classes, transform=None):
        ...
        self.sample_list = [f'{base_dir}/{sample_pth}' for sample_pth in df["image_pth"]]
        self.masks_list = [f'{base_dir}/{sample_pth}' for sample_pth in df["mask_pth"]]
    ...
    def __getitem__(self, idx):
        # if self.split == "train":
        if 'isic' in self.data_dir:
            data = PIL.Image.open(self.sample_list[idx])
            mask = PIL.Image.open(self.masks_list[idx])
            data = np.array(data)
            mask = np.array(mask)
            data = (data - data.min()) / (data.max() - data.min() + 0.00000001)
            image = np.float32(data)
            label = np.float32(np.expand_dims(mask, axis=-1))
        else:
        ...
        if self.split == 'test':
            if 'isic' in self.data_dir:
                ...
                sample['label'] = sample['label'].squeeze(-1)
        sample['case_name'] = self.sample_list[idx].strip('\n')
        # print(sample['image'].shape, sample['label'].shape, sample['low_res_label'].shape)
        return sample
